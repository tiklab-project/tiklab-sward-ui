...
const SharePassWord = (props) => {
    ...
    const jump = ()=> {
        verifyAuthCode({
            ...
            if(data.code===0){
                if(version==="cloud"){
                    props.history.push({pathname: `/share/${props.match.params.shareId}?tenant=${tenant}`, state: {password: "true"}})
                } else {
                    props.history.push({pathname: `/share/${props.match.params.shareId}`, state: {password: "true"}})
                }
            } else {
